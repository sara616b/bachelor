...
import { Title, Text, Container, Button, Flex, Input, Grid, Accordion, TextInput, Box, AccordionControlProps, ActionIcon, FileInput } from '@mantine/core';
...
const App = () => {
    ...
    const savePage = () => {
        ...
        axios
            .post(`/api/page/${pageId}/update`, data)
            .then((response) => {
                console.log(response)
                if (response.status == 200) {
                    getPageInfo()
        ...
        <Container size="xs">
            ...
            >
                ...
                <Accordion chevronPosition="left">
                    ...
                    >
                        {page?.data ? Object.entries(page.data.sections).map(([key, section]) => {
                            console.log(section)
                            return (<div key={`${key}${section.name}`}>
                                <Accordion.Item
                                ...
                                >
                                    ...
                                    <Accordion.Panel><Flex direction="column" gap="md">
                                        <Title order={5}>
                                            Columns
                                        </Title>
                                        {section?.columns ? Object.entries(section?.columns).map(([key, column]) => {
                                            ...
                                            return (<Flex direction="column" gap="md"
                                                ...
                                                <Accordion defaultValue="customization" chevronPosition="left">
                                                    {
                                                        column.components ? Object.entries(column.components).map(([key, component]) => {
                                                            return <EditComponents component={component} index={key} key={`${key}${component.name}`} />;
                ...
                <Flex direction="row" wrap="wrap">
                    <TextInput placeholder='Section Name...' label="Section name" onChange={(event) => setNewSectionName(event.target.value)} />
                    <Button onClick={() => {
                        setPage(
                            {
                                ...page, data: {
                                    sections: [
                                        ...page.data.sections, { name: newSectionName }
                                    ]
                                }
                            }
                        )
